...
export type UpdateStatus =
  ...
  | 'downloading' | 'downloaded' | 'not-available'
...
declare global {
  interface Window {
    electronUpdater?: {
      ...
      onDownloaded:       (cb: (info: UpdateInfo) => void) => void;
      onError:            (cb: (err: { message: string }) => void) => void;
      onRelaunchSoon:     (cb: () => void) => void;
      onManualInstall:    (cb: (data: { path: string | null }) => void) => void;
      startDownload:      () => void;
      installNow:         () => void;
      dismiss:            () => void;
      checkManually:      () => void;
...
export function useAutoUpdater() {
  ...
  useEffect(() => {
    ...
    api.onError((err) =>
      setState((s) => ({ ...s, status: 'error', error: err.message })));
    api.onRelaunchSoon(() => setState(IDLE));
    api.onManualInstall(() =>
      setState((s) => ({ ...s, status: 'manual-install' })));
